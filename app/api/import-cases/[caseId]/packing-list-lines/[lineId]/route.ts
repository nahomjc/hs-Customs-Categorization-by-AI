...
import { db } from "@/db";
import { packingListLines } from "@/db/schema";
...
import { updatePackingListLineSchema } from "@/lib/import-cases/validation";
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  const parsed = updatePackingListLineSchema.safeParse(body);
  ...
  if (!updated) {
    return NextResponse.json(
      { error: "Packing list line not found" },
      { status: 404 },
    );
  }
